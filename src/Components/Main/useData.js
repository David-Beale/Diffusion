...
export const useData = ({ length }) => {
  ...
  const tree = useRef([new Walker({ start: true })]);
  ...
  useFrame(() => {
    while (!w.current.stuck) {
      w.current.walk();
      w.current.checkStuck(tree.current);
    }
    ...
    tree.current.push(w.current.clone());
    w.current = new Walker({
      start: false,
      boundingDist: boundingDist.current,
    });
    index.current++;
    // if (w.current.checkStuck(tree.current)) {
    //   tree.current.push(w.current.clone());
    //   w.current = new Walker({ start: false });
    //   index.current++;
    // }
    meshRef.current.instanceMatrix.needsUpdate = true;
  });
